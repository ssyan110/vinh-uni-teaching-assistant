...
| Pinyin lesson template | Future pinyin decks reuse `output/pinyin/pinyin-01` as the template baseline. Do not build each pinyin lesson from scratch. Keep the same cover/objective/divider/concept/tone/vocabulary/flashcard/practice/input-setup/closing visual language and swap lesson-specific text, taught sounds, vocabulary, exercises, and images. Closing slides use the `下课` visual composition from `/Users/ssyan110/Downloads/Mandarin_Pinyin_Foundations.pptx` while preserving each lesson's original closing text. |
...
| Pinyin image style | Cover, goal, divider, concept, tone, initials/finals, vocabulary, appendix, and closing images use the same soft textbook/course style across all pinyin lessons. Treat them as templates and change only lesson-specific content. For pinyin closing slides, recreate the PPTX `下课` visual in HTML with a pale mint classroom background, large rounded white card, and students leaving through an open classroom door; do not use the full PPTX image as a baked background with old text. |
...
| Vocab image workflow | New slide generation uses blank, correctly sized placeholder images. Generate real vocabulary images only when Adam asks, then insert them with `npm run assets:replace -- --lesson output/book-1/lesson-XX --record V001 --image /path/to/image.png` or crop selected pinyin records from a ChatGPT contact sheet with `npm run assets:crop-contact-sheet -- --lesson output/pinyin/pinyin-XX --sheet /path/to/sheet.png --records V001,V002`. |
| Image asset manifest | Images are tracked as lesson data. VP steps 1-8 should include `image_role`, `image_prompt`, `image_file`, `image_reuse_from`, `image_status`, and `image_semantic_check`. Complete new/regenerated lessons and image-related edits must have `slides/assets/asset-manifest.json` and `exports/qa/asset-qa-report.json`, built with `npm run assets:manifest -- output/book-1/lesson-XX` and checked with `npm run assets:qa -- output/book-1/lesson-XX`. See `docs/image-asset-workflow.md`. |
...
| Overflow QA | For new/regenerated decks, verify every slide for text overflow during HTML/screenshot QA. For small existing-slide edits, verify only the affected slide(s) unless the edit changes shared CSS/templates. Text must not spill outside the slide, background image panel, card, bubble, tile, button, or fixed container. Fix by splitting content into more slides or revising layout. Do not hide clipped text. |
