...
| `pinyin-vocab-contact-sheet-images/` | Pinyin vocabulary image batches: use Google Chrome + ChatGPT to generate one big contact sheet, crop into lesson assets, and QA |
...
- **Classroom presentation:** Open the lesson-root `index.html` in class. Complete lessons launch the HTML presenter at `slides/index.html` — fullscreen 16:9, PowerPoint-style slide thumbnails, pen/highlighter, straight-line, circle, box, text boxes, undo, PDF download with annotations. Database-only draft lessons show a safe status page instead of redirecting to a missing presenter.
...
- **Concise classroom slide rule:** Classroom slides should be `簡潔有力有重點`: one clear title, one clear task, and no extra subtitle/explanation unless Adam explicitly asks for it. Do not fill slides with teacher notes, rule explanations, or redundant helper text. Put those details in teacher prep notes instead.
...
- Annotation tools: pen, highlighter at 30% opacity, straight-line (click two points), circle (drag), box (drag), eraser, transparent draggable text boxes with black/blue/red color and font-size controls, undo, clear
