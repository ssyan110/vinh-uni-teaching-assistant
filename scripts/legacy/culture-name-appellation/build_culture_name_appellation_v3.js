...
const outputDir = process.env.BOYA_CULTURE_DRAFT_DIR
  ...
  : path.join(lessonRoot, '10-design/pptx-draft/social-appellation-supplement-v3');
const assetDir = path.join(outputDir, 'assets');
const pptxPath = path.join(outputDir, '第一课-文化补充-姓名与称呼-v3-draft.pptx');
const outlinePath = path.join(outputDir, '第一课-文化补充-姓名与称呼-v3-outline.md');
const storyboardPath = path.join(outputDir, '第一课-文化补充-姓名与称呼-v3-storyboard.csv');
...
const slideSpecs = [
  ...
  ['13', '称呼', 'surname-role', '姓＋身份', '和越南人的称呼方法有什么不同？'],
  ['14', '称呼', 'familiar', '熟人称呼方式', '说一说：这些称呼适合什么关系？'],
  ['15', '称呼', 'safe', '不确定时，先用安全称呼', '选择一个安全称呼。'],
  ['16', '称呼', 'caution', '注意', '看一看，记住这三个词。'],
  ['17', '称呼', 'roleplay', '情境任务：完成一次第一次见面', '一个人是老师，一个人是学生。'],
  ['18', '称呼', 'scenarios', '你现在会怎么称呼？', '回答五个具体情境：应该怎么称呼？']
];
...
function renderSlide12(pptx, spec) {
  const slide = newStandardSlide(pptx, 13, '姓＋身份', COLORS.paper);
...
function renderSlide13(pptx, spec) {
  const slide = newStandardSlide(pptx, 14, '熟人称呼方式', COLORS.paper);
...
function renderSlide14(pptx, spec) {
  const slide = newStandardSlide(pptx, 15, '不确定时，先用安全称呼', COLORS.paper);
...
function renderSlide15(pptx, spec) {
  const slide = newStandardSlide(pptx, 16, '', COLORS.paper);
...
function renderSlide16(pptx, spec) {
  const slide = newStandardSlide(pptx, 17, '情境任务：完成一次第一次见面', COLORS.paper);
...
function renderSlide17(pptx, spec) {
  const slide = newStandardSlide(pptx, 18, '你现在会怎么称呼？', COLORS.paper);
...
function renderAll() {
  ...
  const renderers = [renderSlide1, renderSlide2, renderSlide3, renderSlide4, renderSlide5, renderSlide6, renderSlide7, renderSlide8, renderSlide9, renderSlide10, renderSlide11, renderSlide12Ask, renderSlide12, renderSlide13, renderSlide14, renderSlide15, renderSlide16, renderSlide17];
...
function writeStoryboard() {
  const rows = [
    ...
    ...slideSpecs.map((row, index) => {
      const assets = [0, 1, 2, 3, 4, 5, 6, 7, 14, 16].includes(index) ? 'generated' : 'shape-only';
