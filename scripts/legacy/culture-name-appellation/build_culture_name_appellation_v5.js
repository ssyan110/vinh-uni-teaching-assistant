...
const outputDir = process.env.BOYA_CULTURE_DRAFT_DIR
  ...
  : path.join(lessonRoot, '10-design/pptx-draft/social-appellation-supplement-v5');
const assetDir = path.join(outputDir, 'assets');
const pptxPath = path.join(outputDir, '第一课-文化补充-姓名与称呼-v5-draft.pptx');
const outlinePath = path.join(outputDir, '第一课-文化补充-姓名与称呼-v5-outline.md');
const storyboardPath = path.join(outputDir, '第一课-文化补充-姓名与称呼-v5-storyboard.csv');
...
const slideSpecs = [
  ...
  ['18', '称呼', 'familiar-name', '熟人称呼方式：名字', '看图，读一读“名字”。'],
  ['19', '称呼', 'familiar-small', '熟人称呼方式：小＋姓／名字', '看图，读一读“小＋姓／名字”。'],
  ['20', '称呼', 'familiar-old', '熟人称呼方式：老＋姓／名字', '看图，读一读“老＋姓／名字”。'],
  ['21', '称呼', 'familiar-a', '熟人称呼方式：阿＋名字', '看图，读一读“阿＋名字”。'],
  ['22', '称呼', 'safe', '不确定时，先用安全称呼', '选择一个安全称呼。'],
  ['23', '称呼', 'caution', '注意', '看一看，记住这三个词。'],
  ['24', '称呼', 'roleplay', '情境任务：完成一次第一次见面', '一个人是老师，一个人是学生。'],
  ['25', '称呼', 'scenarios', '你现在会怎么称呼？', '回答五个具体情境：应该怎么称呼？']
];
...
function renderFamiliarMethodSlide(pptx, page, label, body, imagePath, fill, accent, spec) {
  ...
  addText(slide, body, 6.5, 4.08, 5.48, 0.52, { fontSize: 22, color: accent, bold: true, align: 'center' });
  addNotes(slide, spec, `引导学生先看图片，再读称呼“${label}”；提醒学生根据关系和年龄选择合适的熟人称呼。`);
}

function renderFamiliarName(pptx, spec) {
  renderFamiliarMethodSlide(pptx, 18, '名字', '直接叫名字', images.familiarName, COLORS.mint, COLORS.teal, spec);
}

function renderFamiliarSmall(pptx, spec) {
  renderFamiliarMethodSlide(pptx, 19, '小＋姓／名字', '小王／小明', images.familiarSmall, COLORS.yellowSoft, COLORS.coral, spec);
}

function renderFamiliarOld(pptx, spec) {
  renderFamiliarMethodSlide(pptx, 20, '老＋姓／名字', '老李／老王', images.familiarOld, COLORS.lilac, COLORS.purple, spec);
}

function renderFamiliarA(pptx, spec) {
  renderFamiliarMethodSlide(pptx, 21, '阿＋名字', '阿丽／阿明', images.familiarA, COLORS.coralSoft, COLORS.ink, spec);
}

function renderSlide14(pptx, spec) {
  const slide = newStandardSlide(pptx, 22, '不确定时，先用安全称呼', COLORS.paper);
...
function renderSlide15(pptx, spec) {
  const slide = newStandardSlide(pptx, 23, '', COLORS.paper);
...
function renderSlide16(pptx, spec) {
  const slide = newStandardSlide(pptx, 24, '情境任务：完成一次第一次见面', COLORS.paper);
...
function renderSlide17(pptx, spec) {
  const slide = newStandardSlide(pptx, 25, '你现在会怎么称呼？', COLORS.paper);
...
function renderAll() {
  ...
  const renderers = [renderSlide1, renderSlide2, renderSlide3, renderSlide4, renderSlide5, renderSlide6, renderSlide7, renderSlide8, renderSlide9, renderSlide10, renderSlide11, renderSlide12Ask, renderSlide13SurnameTeacher, renderSlide14SurnameDoctor, renderSlide15SurnameDirector, renderSlide16SurnameStudent, renderSlide17SurnameRoleQuestion, renderFamiliarName, renderFamiliarSmall, renderFamiliarOld, renderFamiliarA, renderSlide14, renderSlide15, renderSlide16, renderSlide17];
...
function writeStoryboard() {
  const rows = [
    ...
    ...slideSpecs.map((row, index) => {
      const assets = [0, 1, 2, 3, 4, 6, 7, 12, 13, 14, 15, 17, 18, 19, 20, 21, 23].includes(index) ? 'generated' : 'shape-only';
