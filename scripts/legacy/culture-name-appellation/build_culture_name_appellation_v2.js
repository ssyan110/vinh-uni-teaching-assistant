...
const outputDir = process.env.BOYA_CULTURE_DRAFT_DIR
  ...
  : path.join(lessonRoot, '10-design/pptx-draft/social-appellation-supplement-v2');
const assetDir = path.join(outputDir, 'assets');
const pptxPath = path.join(outputDir, '第一课-文化补充-姓名与称呼-v2-draft.pptx');
const outlinePath = path.join(outputDir, '第一课-文化补充-姓名与称呼-v2-outline.md');
const storyboardPath = path.join(outputDir, '第一课-文化补充-姓名与称呼-v2-storyboard.csv');
...
const slideSpecs = [
  ...
  ['11', '称呼', 'ask-address', '问称呼的方法', '分清“问姓”和“问称呼”。'],
  ['12', '称呼', 'surname-role', '姓＋身份', '和越南人的称呼方法有什么不同？'],
  ['13', '称呼', 'familiar', '熟人称呼方式', '说一说：这些称呼适合什么关系？'],
  ['14', '称呼', 'safe', '不确定时，先用安全称呼', '选择一个安全称呼。'],
  ['15', '称呼', 'caution', '注意', '看一看，记住这三个词。'],
  ['16', '称呼', 'roleplay', '情境任务：完成一次第一次见面', '一个人是老师，一个人是学生。'],
  ['17', '称呼', 'scenarios', '你现在会怎么称呼？', '回答五个具体情境：应该怎么称呼？']
];
...
function renderSlide11(pptx, spec) {
  const slide = newStandardSlide(pptx, 11, '问称呼的方法', COLORS.paper);
  addCard(slide, 0.84, 1.72, 5.52, 3.86, COLORS.lilac, '问姓', '请问您贵姓？\n\n我姓王。', { titleSize: 25, bodySize: 28, bodyColor: COLORS.ink, bodyBold: true, accent: COLORS.purple, accentW: 2.8, accentH: 0.08, accentX: 1.36, accentY: 1.58, bodyY: 1.18, bodyH: 2.2 });
  addCard(slide, 6.98, 1.72, 5.52, 3.86, COLORS.mint, '问称呼', '怎么称呼您？\n\n叫我王老师就可以。', { titleSize: 25, bodySize: 27, bodyColor: COLORS.ink, bodyBold: true, accent: COLORS.teal, accentW: 2.8, accentH: 0.08, accentX: 1.36, accentY: 1.58, bodyY: 1.18, bodyH: 2.2 });
  addText(slide, '问姓，不等于称呼。', 0.9, 6.08, 11.5, 0.42, { fontSize: 22, color: COLORS.teal, bold: true, align: 'center' });
  addNotes(slide, spec, '让学生比较“问姓”和“问称呼”的不同；本页不放页脚。');
}

function renderSlide12(pptx, spec) {
  const slide = newStandardSlide(pptx, 12, '姓＋身份', COLORS.paper);
  addText(slide, '中国人习惯用“姓＋身份”来称呼别人。', 0.9, 1.58, 11.5, 0.5, { fontSize: 25, color: COLORS.teal, bold: true, align: 'center' });
  const items = [
    ['王老师', COLORS.mint, COLORS.teal],
    ['李医生', COLORS.yellowSoft, COLORS.coral],
    ['张主任', COLORS.lilac, COLORS.purple],
    ['阮同学', COLORS.coralSoft, COLORS.ink]
  ];
  items.forEach(([label, fill, accent], index) => {
    const x = index % 2 === 0 ? 0.88 : 6.84;
    const y = index < 2 ? 2.3 : 3.98;
    addCard(slide, x, y, 5.66, 1.18, fill, label, '', { titleSize: 29, accent, accentH: 0.08, accentW: 2.0, accentX: 1.82, accentY: 0.88, titleY: 0.3 });
  });
  addText(slide, '和越南人的称呼方法有什么不同？', 0.9, 5.9, 11.5, 0.44, { fontSize: 21, color: COLORS.purple, bold: true, align: 'center' });
  addNotes(slide, spec, '提醒学生观察“姓＋身份”的顺序，再和越南人的称呼方法比较。');
}

function renderSlide13(pptx, spec) {
  const slide = newStandardSlide(pptx, 13, '熟人称呼方式', COLORS.paper);
...
function renderSlide14(pptx, spec) {
  const slide = newStandardSlide(pptx, 14, '不确定时，先用安全称呼', COLORS.paper);
...
function renderSlide15(pptx, spec) {
  const slide = newStandardSlide(pptx, 15, '', COLORS.paper);
...
function renderSlide16(pptx, spec) {
  const slide = newStandardSlide(pptx, 16, '情境任务：完成一次第一次见面', COLORS.paper);
...
function renderSlide17(pptx, spec) {
  const slide = newStandardSlide(pptx, 17, '你现在会怎么称呼？', COLORS.paper);
...
function renderAll() {
  ...
  const renderers = [renderSlide1, renderSlide2, renderSlide3, renderSlide4, renderSlide5, renderSlide6, renderSlide7, renderSlide8, renderSlide9, renderSlide10, renderSlide11, renderSlide12, renderSlide13, renderSlide14, renderSlide15, renderSlide16, renderSlide17];
...
function writeStoryboard() {
  const rows = [
    ...
    ...slideSpecs.map((row, index) => {
      const assets = [0, 1, 2, 3, 4, 5, 6, 7, 13, 15].includes(index) ? 'generated' : 'shape-only';
