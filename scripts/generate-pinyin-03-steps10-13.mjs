...
const lessonId = 'pinyin-03';
const lessonRoot = path.join(root, 'output/pinyin/pinyin-03');
const templateRoot = path.join(root, 'output/pinyin/pinyin-01');
const reviewSourceRoot = path.join(root, 'output/pinyin/pinyin-02');
...
const databasePath = path.join(lessonRoot, 'database/vp_pinyin_03_database.json');
...
const reviewMeanings = new Map([
  ['大', 'lớn'], ['拿', 'cầm, lấy'], ['女', 'nữ'], ['绿', 'màu xanh lá'],
  ['地图', 'bản đồ'], ['喝', 'uống'], ['可乐', 'nước cola'], ['哥哥', 'anh trai'],
  ['鸡', 'con gà'], ['几', 'mấy, bao nhiêu'], ['七', 'số bảy'], ['洗', 'rửa'],
  ['西', 'phía tây'], ['骑马', 'cưỡi ngựa'], ['去', 'đi'], ['橘', 'quả quýt'],
  ['机器', 'máy móc, cơ khí'], ['继续', 'tiếp tục'],
]);

const reviewImageFiles = new Map([
  ['大', 'review-v001-da.png'],
  ['拿', 'review-v002-na.png'],
  ['女', 'review-v003-nu.png'],
  ['绿', 'review-v004-lu.png'],
  ['地图', 'review-v005-ditu.png'],
  ['喝', 'review-v006-he.png'],
  ['可乐', 'review-v007-kele.png'],
  ['哥哥', 'review-v008-gege.png'],
  ['鸡', 'review-v009-ji.png'],
  ['几', 'review-v010-ji.png'],
  ['七', 'review-v011-qi.png'],
  ['洗', 'review-v012-xi.png'],
  ['西', 'review-v013-xi.png'],
  ['骑马', 'review-v014-qima.png'],
  ['去', 'review-v015-qu.png'],
  ['橘', 'review-v016-ju.png'],
  ['机器', 'review-v017-jiqi.png'],
  ['继续', 'review-v018-jixu.png'],
]);
...
function coverSlide() {
  return htmlDoc({
    title: 'Pinyin Bài 3',
    body: `<div class="slide"><div class="cover"><div class="cover-card"><div class="lesson">PINYIN 3 · 拼音第三课</div>${coverPinyinTitle()}${coverSoundChips([['zh', 'ch', 'sh', 'r'], ['z', 'c', 's'], 'break', ['一', '不', '→', 'biến điệu']])}</div><div class="topic-img"><img src="assets/photos/cover-topic.png" alt=""></div></div></div>`,
  });
}
...
function objectivesSlide(vocabCount) {
  const goals = [
    ...
    'Học quy tắc <strong class="goal-hot">biến điệu của 一 và 不</strong>',
...
function warmupSlide(reviewItems) {
  ...
  return imageMatchingPracticeSlide({
    title: 'Nối từ Bài 2 với hình ảnh',
    instruction: 'Nối từ Bài 2 với hình ảnh.',
    pairs,
    label: 'Ôn bài 2',
  });
}
...
function chartRows(chart, values = {}) {
  const baseValues = {
    zh: { a: 'zha', e: 'zhe', i: 'zhi', u: 'zhu' },
    ch: { a: 'cha', e: 'che', i: 'chi', u: 'chu' },
    sh: { a: 'sha', e: 'she', i: 'shi', u: 'shu' },
    r: { e: 're', i: 'ri', u: 'ru' },
    z: { a: 'za', e: 'ze', i: 'zi', u: 'zu' },
    c: { a: 'ca', e: 'ce', i: 'ci', u: 'cu' },
    s: { a: 'sa', e: 'se', i: 'si', u: 'su' },
  };
  const tableValues = Object.keys(values).length ? values : baseValues;
...
function chartSlide(chart, title, values = null, label = 'Bảng ghép âm') {
  return slideShell({
    title,
    icon: 'table-2',
    label,
...
function vocabularyGridSlide(set, slideTitle) {
  ...
  return slideShell({
    ...
    icon: 'grid-3x3',
    label: set.title_vi,
    content: `<div class="content"><div class="${vocabularyGridClass(set.items.length)}">${html}</div></div>`,
  });
}
...
function fillBlankPracticeSlide() {
  return slideShell({
    title: 'Điền thanh mẫu còn thiếu',
    icon: 'edit-3',
    label: 'Ôn tập',
    content: renderFillBlankBoard({
      title: 'Điền thanh mẫu còn thiếu.',
      bankTitle: 'Thanh mẫu',
      bankItems: ['zh', 'ch', 'sh', 'r', 'z', 'c', 's'],
      prompts: [
        { initial: 'zh', finalPart: 'á', reveal: true },
        { initial: 'ch', finalPart: 'a' },
        { initial: 'sh', finalPart: 'u' },
        { initial: 'r', finalPart: 'e' },
        { initial: 'z', finalPart: 'i' },
        { initial: 'c', finalPart: 'a' },
        { initial: 's', finalPart: 'i' },
        { initial: 'zh', finalPart: 'u' },
        { initial: 'ch', finalPart: 'i' },
        { initial: 'sh', finalPart: 'i' },
        { initial: 'z', finalPart: 'e' },
        { initial: 's', finalPart: 'udu' },
...
function closingSlide() {
  return htmlDoc({
    title: 'Kết thúc',
    body: `<div class="slide closing"><div class="closing-card-ref"><img class="closing-door" src="assets/reference/closing-door-students.png" alt=""><div class="closing-zh">下课</div><div class="closing-sub">Bạn có câu hỏi gì không?</div><div class="closing-next">Bài tiếp theo: Pinyin 4</div></div></div>`,
  });
}
...
<head>
  ...
  <title>Pinyin Bài 3</title>
  <script>window.location.replace('slides/index.html');</script>
</head>
<body>
  <p><a href="slides/index.html">Mở Pinyin Bài 3</a></p>
</body>
</html>
`;
}
...
async function writeSlides(db, vocab, sets) {
  ...
  const chart1 = db.pinyin_charts.find((chart) => chart.item_id === 'pinyin_chart_l3_group_1');
  const chart2 = db.pinyin_charts.find((chart) => chart.item_id === 'pinyin_chart_l3_group_2');
  const set1 = sets[0];
  const set2 = sets[1];
  const toneRetroflex = {
    zh: { a: 'zhā', e: 'zhé', i: 'zhǐ', u: 'zhù' },
    ch: { a: 'chá', e: 'chē', i: 'chī', u: 'chū' },
    sh: { a: 'shǎ', e: 'shé', i: 'shì', u: 'shū' },
    r: { e: 'rè', i: 'rì', u: 'rǔ' },
  };
  const toneDental = {
    z: { a: 'zā', e: 'zé', i: 'zì', u: 'zǔ' },
    c: { a: 'cā', e: 'cè', i: 'cǐ', u: 'cù' },
    s: { a: 'sā', e: 'sè', i: 'sì', u: 'sù' },
  };
...
async function writeTeacherGuide(db, vocab, sets, slideFiles) {
  const guide = `# Pinyin Bài 3 - Teacher Prep

## Mục tiêu

- Ôn lại từ vựng Pinyin Bài 2 bằng nghe, đọc và hiểu nghĩa.
- Dạy thanh mẫu \`zh ch sh r\`, sau đó \`z c s\`.
- Dạy quy tắc \`一 / 不 biến điệu\`: tập trung vào cách đọc khi nói, không biến thành bài lý thuyết dài.
- Chốt bảng tổng hợp 21 thanh mẫu đã học qua Pinyin Bài 1-3.
...
2. Ôn nhanh Bài 2.
3. Dạy nhóm thanh mẫu 1: \`zh ch sh r\`.
4. Ghép âm và luyện đọc có thanh điệu; nhấn mạnh lưỡi cuốn sau.
5. Từ vựng 1: ${sets[0].items.map((item) => `${item.chinese_simplified} ${item.pinyin}`).join(', ')}.
6. Dạy nhóm thanh mẫu 2: \`z c s\`.
7. Ghép âm và luyện đọc có thanh điệu.
8. Từ vựng 2: ${sets[1].items.map((item) => `${item.chinese_simplified} ${item.pinyin}`).join(', ')}.
9. Quy tắc \`一 / 不 biến điệu\` bằng ví dụ ngắn.
10. Bảng tổng hợp 21 thanh mẫu.
11. Ôn tập nghe chọn pinyin, điền thanh mẫu và ghép nghĩa.
...
`;
  ...
  const brief = `# Huashu Brief - Pinyin Bài 3

## Deck

- Output: \`output/pinyin/pinyin-03/slides/\`
...
async function writeReadme(slideFiles) {
  await fs.writeFile(path.join(lessonRoot, 'README.md'), `# Pinyin Bài 3
