...
export function renderFillBlankBoard({ title, bankTitle, bankItems, prompts }) {
  return `<div class="content"><div class="listen-head"><div class="listen-title">${esc(title)}</div></div><div class="final-bank"><div class="final-bank-title">${esc(bankTitle)}</div><div class="finals">${bankItems.map((item) => `<span>${esc(item)}</span>`).join('')}</div></div><div class="listen-grid">${prompts.map((prompt, i) => `<div class="listen-item"><span class="listen-no">${i + 1}.</span>${esc(prompt.initial)}<span class="blank${prompt.short ? ' short' : ''}"></span></div>`).join('')}</div></div>`;
}
