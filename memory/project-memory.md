...
- Verify with concrete checks: pipeline/data validation, asset manifest + asset QA, HTML presenter smoke tests, browser screenshots/contact sheets, no-overflow review, vocab image match review, and PDF export only after Adam confirms finalization.
...
- Use `npm run assets:manifest -- output/book-1/lesson-XX`, `npm run assets:qa -- output/book-1/lesson-XX`, and `npm run assets:replace -- --lesson output/book-1/lesson-XX --record V001 --image /path/to/new.png` for image workflows.
