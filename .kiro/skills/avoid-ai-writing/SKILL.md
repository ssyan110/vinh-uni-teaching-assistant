...
## Resume / CV audit addendum

When auditing a resume or CV, use a resume-specific pass instead of treating it like blog prose:

- Preserve bullet structure. Resumes are supposed to be scannable, so do not convert bullet-heavy sections into paragraphs.
- Prioritize credibility over personality. The main AI tells in CVs are repeated verbs, inflated impact claims, buzzword clustering, and vague adjectives.
- Flag repeated lead verbs across adjacent bullets, especially `Implemented`, `Enhanced`, `Contributed`, `Built`, and `Managed`. Suggest more specific verbs only where they match the actual work.
- Replace vague impact language with either concrete metrics or safer wording. Example: `help reduce user drop-off` → `make the application journey easier to complete`, unless a real drop-off metric is available.
- Treat trendy AI tooling lines carefully. Keep them if they describe a real workflow, but rewrite hype like `accelerating feature delivery and code quality` into practical behavior such as `reuse project context`, `generate code faster`, or `review implementation patterns more consistently`.
- Keep technical terms that are normal for the target role. Do not flag legitimate engineering vocabulary such as `Clean Architecture`, `monorepo`, `server-driven UI`, `platform channels`, `Crashlytics`, or `Sentry`.
- For final feedback, include: high-level assessment, specific text to adjust, suggested rewrites for the strongest sections, and any layout/content risks.
...
**`resume-cv`** � Resume/CV and candidate profile writing. Preserve truthful metrics, tech stack, company/product names, and role scope; focus on recruiter readability. Flag AI tells that show up as inflated impact claims, repeated action verbs, vague adjectives, over-packed bullets, and trendy tooling language without evidence.
...
| Rule | linkedin | resume-cv | blog | technical-blog | investor-email | docs | casual |
|------|----------|-----------|------|----------------|----------------|------|--------|
...
| Excessive bullets | skip (lists work on LinkedIn) | relaxed (CVs need bullets; trim only overlong or redundant bullets) | strict | relaxed (technical lists OK) | strict | skip (lists are docs) | skip |
| Hedging | strict | strict | strict | relaxed ("may" is accurate in technical) | strict | relaxed | skip |
| Word table (full list) | strict | strict for inflated/buzzword claims; allow standard technical nouns | strict | **partial** (see below) | strict | relaxed | P0 only |
| Promotional language | relaxed (some sell is expected) | strict | strict | strict | **extra strict** | strict | skip |
| Significance inflation | strict | strict | strict | strict | **extra strict** | relaxed | skip |
| Copula avoidance | skip | strict | strict | relaxed | strict | skip | skip |
| Uniform paragraph length | skip (short-form) | skip (bullet format dominates) | strict | strict | strict | relaxed | skip |
| Numbered list inflation | relaxed | strict | strict | relaxed | strict | skip | skip |
...
| Generic conclusions | skip | strict | strict | strict | **extra strict** | skip | skip |
...
### Resume/CV audit guidance

When auditing a resume or CV, use `resume-cv` unless the user asks for another profile. The goal is not to make it more casual; it is to make every bullet credible, specific, and easy for a recruiter or hiring manager to defend in an interview.

Prioritize these fixes:
- Repeated action verbs: if several neighboring bullets start with the same verb (`Implemented`, `Enhanced`, `Contributed`, `Built`), vary the verbs or combine related bullets.
- Over-packed bullets: split bullets that contain two unrelated achievements joined by semicolon or multiple commas. One bullet should usually carry one result or one coherent technical contribution.
- Vague impact claims: replace `helped reduce`, `accelerating`, `improving quality`, `reliable`, `seamless`, `robust`, or similar claims with a number, a concrete workflow change, or a neutral description. If no metric exists, use direct wording such as `improved the onboarding flow` rather than pretending to prove impact.
- Trendy tooling language: keep AI tools, agents, prompt engineering, and MCP only when tied to a real workflow. Phrase them as engineering practice (`set up project rules`, `reused context`, `reviewed implementation patterns`) rather than hype.
- Technical credibility: preserve exact technologies, architecture patterns, integrations, app names, user counts, and security/compliance details unless they are unsupported or redundant.
- CV density: bullets are allowed and expected. Do not convert a resume into prose. Trim weak bullets before changing the format.

Good resume rewrites sound plain and defensible: `Worked with backend, QA, security, and compliance teams to debug production issues and ship features for a 4M+ user fintech app.` Avoid inflated versions such as `Partnered cross-functionally to deliver reliable fintech features at scale` unless backed by specifics.
