...
7. **验证**：用Playwright截图（见 `references/verification.md`），检查控制台错误，发给用户。
   - 🛑 **社媒 carousel / feed / story 导出硬要求**：必须先生成最终 PNG，再做 contact sheet 肉眼 QA；不能只看 HTML 或单页预览就交付。
   - PNG 尺寸必须匹配目标平台的最终像素（常用：1:1 = 1080×1080；4:5 = 1080×1350；9:16 = 1080×1920）。Playwright `deviceScaleFactor` 会改变导出像素；若 CSS viewport 已是最终尺寸，设 `deviceScaleFactor: 1`，避免导出 2160×2700/2160×3840 这种非目标尺寸。
   - QA 必查：所有文字在边界内；无截断/溢出；顶部品牌/页码和底部 CTA 避开平台 UI；中文繁/简、拼音、越南文都按真实可读文本处理；密集比较页宁可拆页，不要把字号压到手机不可读。
   🛑 **检查点4：交付前自己肉眼过一遍浏览器/PNG contact sheet**。AI写的代码经常有interaction bug；长文 carousel 的问题常只在最终截图里暴露。
...
| **Marketing 领域任务**（广告创意/落地页/社媒/活动横幅/品牌物料） | `references/domains/marketing-guide.md` |
| **Teaching 领域任务**（讲座幻灯片/课程封面/教育信息图/教学动画） | `references/domains/teaching-guide.md` |
| **AI/ML Engineering 领域任务**（架构图/模型仪表盘/研究海报/API文档/Demo原型） | `references/domains/aiml-guide.md` |
| **领域配置文件**（设计 token / 风格推荐 / 场景模板 / 工作流预设 / 触发词） | `references/domains/{domain}-profile.md`（marketing / teaching / aiml） |
| **Tiếng Hoa HAHA 内容任务**（轮播图/字幕叠加/词汇卡/缩略图/故事卡） | `references/domains/tienghoahaha-profile.md` |
| **Tiếng Hoa HAHA 内容格式规范** | `references/tieng_hoa_haha_content_format_guide.md` |
| **Tiếng Hoa HAHA 品牌语气参考** | `references/tienghoahaha_content/reference_past_social_media_data/adam-writing-tone-report.md` |

## Domain Customization · 领域定制

> **纯增量层**：领域定制不禁用任何现有能力。所有现有规则（反 AI slop、核心资产协议、Junior Designer 模式、事实验证）在领域激活后**完全保持生效**。领域只是收窄默认推荐（推荐哪些风格、优先哪些模板、预填哪些工作流参数），不限制用户选择。

### 可用领域

| 领域 | 激活命令 | 配置文件 | 参考指南 | 样例目录 |
|------|---------|---------|---------|---------|
| **Marketing** 营销 | `marketing mode` / `营销模式` | `references/domains/marketing-profile.md` | `references/domains/marketing-guide.md` | `assets/showcases/marketing-ad/` · `marketing-landing/` · `marketing-banner/` |
| **Teaching** 教学 | `teaching mode` / `教学模式` | `references/domains/teaching-profile.md` | `references/domains/teaching-guide.md` | `assets/showcases/teaching-slide/` · `teaching-infographic/` · `teaching-cover/` |
| **AI/ML Engineering** AI/ML 工程 | `AI/ML mode` / `AI/ML模式` | `references/domains/aiml-profile.md` | `references/domains/aiml-guide.md` | `assets/showcases/aiml-architecture/` · `aiml-dashboard/` · `aiml-poster/` |
| **Tiếng Hoa HAHA** 越中语言教育 | `haha mode` / `tiếng hoa haha mode` | `references/domains/tienghoahaha-profile.md` | `references/tieng_hoa_haha_content_format_guide.md` | — (使用 profile 内场景模板) |

### 触发词列表（Domain Router 自动检测用）

| 领域 | 触发词 |
|------|--------|
| **Marketing** | `ad creative` · `landing page` · `campaign` · `social media post` · `brand material` · `conversion` · `CTA` · `banner` · `广告` · `落地页` · `活动` · `社媒` · `品牌物料` · `转化` |
| **Teaching** | `lecture` · `course` · `tutorial` · `student` · `educational` · `lesson plan` · `classroom` · `curriculum` · `讲座` · `课程` · `教程` · `学生` · `教学` · `课件` |
| **AI/ML Engineering** | `model architecture` · `neural network` · `data pipeline` · `research paper` · `API docs` · `ML dashboard` · `training metrics` · `inference` · `模型架构` · `神经网络` · `数据管线` · `研究论文` |
| **Tiếng Hoa HAHA** | `tiếng hoa` · `tiếng trung` · `haha` · `tienghoahaha` · `carousel` · `vocabulary card` · `flashcard` · `pinyin` · `4-line` · `subtitle overlay` · `轮播图` · `词汇卡` |
...
用户 Prompt 到达
  │
  ├─ 包含显式激活命令？（"marketing mode" / "teaching mode" / "AI/ML mode" / "haha mode"）
  │   └─ YES → 直接激活对应领域，加载 profile
  │
  ├─ 扫描触发词
  │   ├─ 仅匹配单一领域 → 建议该领域，问用户确认
  │   │   └─ 用户确认 → 激活；用户拒绝 → 标准流程（无领域）
  │   ├─ 匹配多个领域 → 列出所有匹配领域，让用户选择
  │   │   └─ 用户选择 → 激活所选领域
  │   └─ 无匹配 → 标准流程（无领域），不报错
  │
  └─ 领域激活后
      ├─ 读取 references/domains/{domain}-profile.md
      ├─ 注入 Design Token Set（:root { --domain-* }）
      ├─ 风格推荐从领域 Style Map 中抽取（而非通用库）
      ├─ 场景模板优先领域专属模板
      ├─ 工作流预设预填 Junior Designer 问题
      └─ Phase 4 样例画廊优先展示领域匹配样例
```
...
| 环节 | 无领域（标准） | 有领域（激活后） |
|------|--------------|----------------|
| **Phase 3 风格推荐** | 从 20 种通用风格库推荐 3 个方向 | 从领域 Style Map 推荐 3 个方向（仍保证 3 个不同流派） |
| **Phase 4 样例画廊** | 展示 24 个通用样例 | **优先**展示领域匹配样例，通用样例仍可浏览 |
| **Junior Designer 问问题** | 逐项问 fidelity / variations / Tweaks / format | 用 Workflow Preset 预填默认值，展示给用户确认 |
| **"skip questions" / "直接做"** | 必须逐项问 | 使用 Preset 默认值直接执行，assumptions 写入 HTML 注释 |
| **HTML 输出** | 使用品牌 token 或通用 token | 注入领域 `--domain-*` CSS 变量到 `:root` |
| **场景模板** | 从通用模板库匹配 | 优先领域专属模板，通用模板仍可用 |
| **交付前验证** | 通用质量检查 | 额外参考领域 Reference Guide 的验证清单 |

### 领域资源文件路径汇总

```
references/domains/
├── marketing-profile.md      # Marketing 领域配置（token + 风格 + 模板 + 预设 + 触发词）
├── marketing-guide.md        # Marketing 参考指南（平台规格 + 最佳实践 + 反模式 + 验证清单）
├── teaching-profile.md       # Teaching 领域配置
├── teaching-guide.md         # Teaching 参考指南
├── aiml-profile.md           # AI/ML 领域配置
└── aiml-guide.md             # AI/ML 参考指南

references/
├── tieng_hoa_haha_content_format_guide.md  # Tiếng Hoa HAHA 内容格式规范
└── tienghoahaha_content/                    # Tiếng Hoa HAHA 品牌资料库
    ├── content_format_guide.md              # 内容格式指南（源文件）
    └── reference_past_social_media_data/    # 历史社媒数据（FB/IG/TikTok/YouTube）
        └── adam-writing-tone-report.md      # Adam 写作语气报告

assets/showcases/
├── marketing-ad/             # 广告创意样例（Pentagram + Build）
├── marketing-landing/        # 落地页首屏样例（Pentagram + Build）
├── marketing-banner/         # 活动横幅样例（Sagmeister + Locomotive）
├── teaching-slide/           # 讲座幻灯片样例（Fathom + Kenya Hara）
├── teaching-infographic/     # 教育信息图样例（Müller-Brockmann + Takram）
├── teaching-cover/           # 课程封面样例（Kenya Hara + Build）
├── aiml-architecture/        # 架构图样例（Fathom + Territory）
├── aiml-dashboard/           # 模型仪表盘样例（Fathom + Pentagram）
└── aiml-poster/              # 研究海报样例（Müller-Brockmann + Takram）
```

### 重要：增量行为

- 领域激活**不会**禁用任何现有风格、模板或工作流步骤
- 用户可以在领域激活后请求领域推荐之外的风格——系统会尊重请求，不做限制（Requirement 3.5）
- 用户可以随时切换领域或退出领域模式
- 领域 token 使用 `--domain-*` 命名空间，与品牌 token `--brand-*` 互不冲突，可共存
