...
- Pinyin images: cover, goal, divider, concept, tone, initials/finals, vocabulary, appendix/input setup, and closing images all use the same soft textbook/course style across pinyin lessons. Treat them as templates; change only lesson-specific content and image subject. For pinyin closing slides, recreate the PPTX `下课` visual in HTML with a pale mint classroom background, large rounded white card, and students leaving through an open classroom door; do not use the full PPTX image as a baked background with old text.
...
- Slide style selection (required gate): before generating any lesson slide deck, ask Adam which visual style to use: `original`, `coral-studio`, or `slate-citrus`. Default is Slate Citrus when Adam doesn't specify. Styles are visual-only CSS skins over the locked layouts; reference skins live at `output/pinyin/pinyin-01/design-prototypes/<style>-full-deck/prototype.css` (`original` = no extra skin). Apply by copying the skin to the lesson's `slides/assets/style-<name>.css` and linking it as the last stylesheet in every slide, after `slide-base.css` and the inline style block. Never change layout geometry, content, ordering, or presenter behavior when applying a style. Comparison picker: `output/pinyin/pinyin-01/design-options/index.html`.
